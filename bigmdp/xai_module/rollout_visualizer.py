...
# def simple_rollout_visualizer(env, myAgent, policy_key= "optimal", eps_count=1, lag=0, render=False, action_meanings=None):
#
#     get_nn_fxn = myAgent.mdp_T._get_nn_hs_kdtree
#     qD = cpy(myAgent.mdp_T.qvalDict)
#     vD = cpy(myAgent.mdp_T.valueDict)
#     tD = myAgent.mdp_T.tD
#     A = myAgent.mdp_T.A
#     rD = myAgent.mdp_T.rD
#     policy = myAgent.policies[policy_key]
#     action_meanings = action_meanings or [str(a) for a in A]
#     abstraction_fxn = myAgent.mdp_T.encode_single
#
#     video_list = []
#     total_reward = 0
#     all_rewards = []
...
#         prev_state_in_mdp = hs_d in myAgent.mdp_T.s2idx
#
#
#         while True:
#             # Get an action
...
#             this_state_in_mdp = hns_d in myAgent.mdp_T.s2idx
#
#             # print("#" * 40)
#             # print(hs_d, hns_d)
...
#             s_match, a_match, ns_match = prev_state_in_mdp, a in tD[nn_hs_d], this_state_in_mdp
#             t_exact_match = hns_d in tD[hs_d][a]
#             t_nn_match = not t_exact_match and nn_hns_d in tD[nn_hs_d][a]
#
...
#             to_print_dict["State Id"] = str(myAgent.mdp_T.s2idx[nn_hs_d])
#             to_print_dict["S_In_MDP"] = str(s_match) + " ->(" + str(a_match) + ")->" + str(ns_match)
#             to_print_dict["Tran_prob"] = round(myAgent.mdp_T.tD[nn_hs_d][a][nn_hns_d], 4) if t_exact_match or  t_nn_match else "----"
#             to_print_dict["pred_reward"] = round(rD[nn_hs_d][a], 4)
# #             to_print_dict["# Actions"] = str(mdp.get_seen_action_count(nn_hs_d))
#             to_print_dict["Best Axn"]= str(action_meanings[a])
#             to_print_dict["VI State Value"] = str(round(vD[nn_hns_d], 4))
#             to_print_dict["Exp VI (s,a) value"] = round(qD[nn_hs_d][a], 4)
...
#             value_history.append(vD[nn_hns_d])
#
...
#
#             # other tracking metrics
#             value_history.ap pend(v_dict[nn_hns_d])
#
#             # Update counts for next iteration
#             prev_state_in_mdp = this_state_in_mdp
#             hs_d = hns_d
